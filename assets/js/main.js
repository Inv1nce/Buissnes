...
(function () {
  ...
  /* ---------------------------------------------------------
     ...
     Кадры нарезаны из assets/parrot-sheet.png скриптом
     docs/build-parrot-frames.py. Сетка 3 на 2, порядок:
       0  полёт, крылья вверху
       1  полёт, мах вниз
       2  полёт, крылья внизу
       3  сидит спокойно
       4  чистит перья
       5  смотрит вверх
     Лапы во всех кадрах стоят на одной высоте — 0,78 от верха ячейки.
     --------------------------------------------------------- */
  var PARROT = {
    sprite: 'assets/parrot.png',
    cols: 3,
    rows: 2,
    feetLine: 0.78,        // где в кадре лапы, долей от высоты ячейки
    clips: {
      fly:   { from: 0, to: 2, fps: 8, loop: true, pingpong: true },
      land:  { from: 2, to: 3, fps: 4 },
      idle:  { from: 3, to: 3 },
      look:  { from: 5, to: 5 },
      preen: { from: 3, to: 4, fps: 1.6 }
    }
  };
  ...
  function initParrot() {
    ...
    function build() {
      ...
      /* Птица садится лапами на верхнюю кромку элемента: центр кадра
         поднимаем над кромкой на разницу между линией лап и центром. */
      var size = parrot.offsetWidth || 68;
      var feetLift = size * ((PARROT.sprite ? PARROT.feetLine : 0.81) - 0.5);

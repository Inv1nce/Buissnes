...
### 2. Попугай — уже на месте

В `assets/parrot.png` лежит лента из шести кадров, нарезанная из
`assets/parrot-sheet.png` скриптом `docs/build-parrot-frames.py`.

Сетка 3 на 2, порядок кадров:

| № | Кадр | № | Кадр |
|---|---|---|---|
| 0 | полёт, крылья вверху | 3 | сидит спокойно |
| 1 | полёт, мах вниз | 4 | чистит перья |
| 2 | полёт, крылья внизу | 5 | смотрит вверх |

Лапы во всех кадрах стоят на одной высоте — 0,78 от верха ячейки.
На эту величину опирается посадка на элементы, она прописана
в `PARROT.feetLine`.

Если будете перевыпускать графику, положите новый лист в
`assets/parrot-sheet.png`, поправьте координаты поз в начале
`docs/build-parrot-frames.py` и запустите его — он сам снимет фон,
отзеркалит сидящие позы, выровняет по лапам и соберёт ленту.

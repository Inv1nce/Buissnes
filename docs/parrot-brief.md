...
## 9. Кадры собраны, попугай на сайте

Лист со второго прогона нарезан и подключён. Что пришлось сделать
и что важно знать на будущее.

**Прозрачности в файле не было.** Серая шахматка оказалась обычными
пикселями, альфа-канал был полностью непрозрачным. Снимал фон
программно: шахматка — это светлые пиксели без насыщенности
(разброс между каналами до 11), а вся графика насыщенная. Кремовая
щека птицы тёплая, разброс около 20, поэтому под правило не попала.
Дополнительно фон отбирался только тот, что связан с краем картинки, —
иначе блик в глазу, похожий на белый, тоже стал бы дырой.

**Подписи и выноски убраны** двумя приёмами: в каждом кадре оставлен
только самый большой связный кусок, а тонкие линии срезаны
морфологическим открытием ядром 9 на 9 — лапы и клюв толще, они уцелели.
Отдельно стёрты нейтрально-тёмные пиксели: подписи серые, а клюв
и зрачок тёплые, по этому признаку они и различаются.

**Сидящие позы отзеркалены**: на листе они смотрели влево, полётные —
вправо. Теперь все смотрят вправо.

**Выравнивание.** Сидящие кадры совмещены по лапам: низ лап на одной
высоте, центр по горизонтали общий. Полётные кадры совмещены по центру
кремовой щеки, причём щека поставлена туда же, где она у спокойной
позы, — поэтому при посадке птица не прыгает.

Итог: лента 960 на 640, ячейка 320, вес 207 КБ.

Чего в наборе нет и что можно дорисовать позже: отдельного кадра
моргания и промежуточных кадров посадки. Сейчас посадка — это переход
от кадра «крылья внизу» к кадру «сидит», двух кадров для неё хватает.
